...
**Rotation announcement wire format:** See [wire-format-spec.md § Rotation Announcement](wire-format-spec.md#0x02--rotation-announcement) for the 201-byte byte layout. The 136-byte signable payload includes old/new keys (4 × 32 bytes) and a timestamp (8 bytes), signed with an Ed25519 signature by the old key.
...
- **Default behavior (`softRepin`):** If a previously-pinned peer appears with a different public key (e.g., user reinstalled the app), the new key is silently accepted and re-pinned. The `onKeyChanged(peer, oldKey, newKey)` callback fires for app-level awareness — the app can alert the user or log the event. This prioritizes UX for typical use cases (festivals, hiking, casual chat) where app reinstalls are common.
- **Strict mode (`strict`):** Messages from that peer are **rejected**. The library invokes `onKeyChanged(peer, oldKey, newKey)` and the consuming app must call **`repinKey(peer)`** to explicitly accept. This follows the SSH `known_hosts` model and is recommended for high-security deployments (enterprise, activism). Enabled via `trustMode = TrustMode.STRICT`.
...
**Tradeoff:** `softRepin` prioritizes usability — a peer reinstalling their app doesn't break communication with the entire mesh. `strict` mode is available for apps in adversarial environments.
