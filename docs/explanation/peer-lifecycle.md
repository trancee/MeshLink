...
The `CONNECTED → DISCONNECTED → GONE` model describes **unexpected** loss: interference, movement, OS suspension, or a dropped BLE link. An intentional local stop is different because the departing peer knows it is leaving.

On the live Android BLE proof path, waiting only for platform disconnect callbacks was not enough. Physical three-device evidence showed that disconnects could be asymmetric: Bob's client-side disconnect reached one neighbor while Bob's server-side disconnect lagged long enough for another neighbor to keep stale reachability and routes.

The current Android proof/runtime path now does this on an intentional stop:

1. The stopping peer broadcasts a runtime-private `RouteWithdrawal(origin = self, destination = self)` to each direct peer.
2. A receiver treats `destination == nextHop` as "the direct peer itself is leaving," drops that direct peer, withdraws every route learned via it, and emits `PeerEvent.Lost` for the direct peer and each affected transitive route.
3. BLE disconnect callbacks and sweep-based cleanup still run afterward, but they are now backstops instead of the only convergence path.

This keeps the public API unchanged. The explicit self-withdrawal is private control-plane behavior inside MeshRuntime, built on the same runtime-private relay envelope used for route advertisement and withdrawal.
