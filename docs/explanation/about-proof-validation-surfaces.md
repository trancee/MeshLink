...
|---|---|---|---|
| Reference app | product-like evaluation | supported session flow, operator-facing diagnostics, retained history, and export behavior | transport-fixture benchmarking and prototype transport investigation |
| Proof apps | physical transport fixtures | device-to-device proof runs, second-device peers, and proof-only transport experiments | the supported product-like evaluation surface |
...
The proof apps are transport-validation fixtures, not smaller copies of the
reference app.
...
- `meshlink` is the product-path runtime and is the right default when you need
  a tutorial peer, a manual proof peer, or a sanity check close to supported
  behavior
...
- the current Android `gatt` prototype is a passive server-oriented fixture
- the current iPhone prototype modes are active benchmark modes rather than
  passive observation fixtures
...
| use an Android device as a physical proof peer or transport fixture | [How to run the Android proof app](../../meshlink-proof/android/README.md) |
| use an iPhone as a physical proof peer or transport fixture | [How to build and run the iOS proof app](../../meshlink-proof/ios/README.md) |
