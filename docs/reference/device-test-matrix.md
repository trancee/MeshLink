...
| Xiaomi Pocophone F1 | Xiaomi | POCOPHONE F1 | Android 10 / SDK 29 | 6 GB | 64 GB | 5.0 | 6.18" | Snapdragon 845 | MIUI 12 | ChaCha20-Poly1305 | Oldest Android version in the set; useful for legacy app compatibility and MIUI-specific behavior. Runtime probe on the attached handset reported `x25519=false`, `ed25519=false`, `chacha=true`, `meshRuntime=false`, and `provider=AndroidFallbackCryptoProvider`. | [GSMArena](https://www.gsmarena.com/xiaomi_pocophone_f1-9293.php) |
...
The crypto column records the runtime probe result for the device's JCA
stack. `CryptoRuntimeValidationDeviceTest` prints the measured
`x25519`/`ed25519`/`chacha` booleans and the selected provider so the table can
show what the attached handset actually exposes. It is not a guarantee of
hardware-backed key storage. Bluetooth LE link security also uses AES-CCM under
the hood; hardware acceleration and vendor keystore behavior can still vary by
