...
Before selecting hardware, check the [Device test matrix reference](../reference/device-test-matrix.md) for the current attached fleet, RAM, storage, Bluetooth version, and known quirks. The current bench includes the new Motorola Edge 30 Fusion, Nokia X20, and OnePlus Nord 2 5G, so use those when you need a fresh Android 14/13 mix in the same physical campaign.
