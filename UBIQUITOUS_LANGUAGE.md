...
| **DiagnosticEvent** | Structured event from the **Diagnostic Stream**: `{code: DiagnosticCode, severity: Severity, monotonicMs, wallClockMs, droppedCount, payload: Map<String, Any>}`. Four severity levels: `FATAL` (terminal), `ERROR` (app should act), `WARNING` (degraded), `INFO` (observability). Apps filter by severity without knowing every code. | Log event, debug event |
| **Diagnostic Timestamp** | Each `DiagnosticEvent` carries dual timestamps: `monotonicMs` (millis since boot — `System.nanoTime()/1M` on Android, `ProcessInfo.systemUptime×1000` on iOS) for interval calculations and cross-event ordering; `wallClockMs` (epoch millis — `System.currentTimeMillis()` on Android, `Date().timeIntervalSince1970×1000` on iOS) for human-readable log correlation. 16 bytes per event. | Event time, timestamp |
