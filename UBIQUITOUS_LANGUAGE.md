...
| **TOFI** | Trust-on-First-Discover — the trust model where a **Peer's** **Public Key** is pinned on first encounter. Default mode is **strict**: key changes are rejected until the consuming app explicitly re-pins. | TOFU (different: that's Trust On First Use, a related but distinct term) |
...
| **Write-Ahead Replay Counter** | The **outbound** Replay Counter, persisted on every increment (not periodically). Eliminates counter gaps after unexpected termination. The **inbound** replay state uses periodic persistence (every 1s) — see **Replay Window**. This asymmetry is intentional: outbound counter gaps would permanently desync a session, while inbound window gaps only cause brief duplicate acceptance. | Eager-persist counter |
| **Persistence Failure Policy** | Criticality-based disk write failure handling: **Identity**/TOFI → `fatalError(retryable: true)` after 3 retries; **Replay Counter** → retry 3×, then `PERSIST_FAILED` diagnostic + continue in-memory; **Dedup Set** → in-memory only in v1 (no persistence). Pre-flight `LOW_DISK_SPACE` diagnostic if <1MB free. | Storage failure, disk full handling |

## Transport

| Term | Definition | Aliases to avoid |
|------|-----------|-----------------|
| **Advertisement** | A BLE advertisement payload (17 of 24 usable bytes): bit-packed protocol version + **Power Mode** (1 byte) and **Key Hash** (16 bytes). | Beacon, broadcast (avoid — "Broadcast" means something specific in messaging) |
...
| **L2CAP Backpressure** | Condition where L2CAP write blocks >100ms due to credit exhaustion. **Fallback trigger:** 3 consecutive >100ms blocks within `l2capBackpressureWindow` (default 7s, configurable 3–15s) → **GATT Fallback** activated, `resume_request` sent. L2CAP NOT retried until next fresh connection. | L2CAP stall, credit stall |
...
| **MTU Negotiation** | The BLE GATT MTU exchange that occurs after connection. The OS negotiates the largest MTU both devices support. If the effective payload (`negotiated_MTU − 4` for ATT overhead + type prefix) is < 100 bytes (i.e., `negotiated_MTU < 104`), the connection is rejected with `MTU_TOO_SMALL`. Broadcasts via GATT require MTU ≥ 119 (envelope overhead = 118 bytes). | MTU exchange, MTU handshake |
...
| **Message ID** | A 128-bit UUID v4 randomly generated by the sender, uniquely identifying a message across the mesh for deduplication. | Message hash, packet ID |
...
| **Handshake Rate Limiting** | Sybil mitigation: new **Noise XX Handshakes** with unknown **Peers** are rate-limited to 1 per second. **TOFI**-pinned peers are exempt. Forces Sybil devices to spend real time establishing connections. Configurable via `handshakeRateLimit`. | Connection throttle, handshake throttle |
...
| **Route Error (RERR)** | *(Deferred to post-v1; routes expire naturally via **Route Expiry** at 5× **Gossip Interval**.)* | Route failure, link break notification |
...
| **Configuration Preset** | Factory method returning `MeshLinkConfig` with tuned defaults for a use case: `chatOptimized()` (low latency), `fileTransferOptimized()` (large payloads), `powerOptimized()` (battery conservation). All values individually overridable via DSL block. | Config template, preset |
| **Diagnostic Backpressure** | **Diagnostic Stream** bounded buffer (capacity 256, DROP_OLDEST). Slow subscribers lose old events, never block mesh engine. Each `DiagnosticEvent` carries a `droppedCount: Int` field — `0` when no drops, `N` when N events were dropped since the last emitted event. Zero-cost overflow detection via atomic counter. | Diagnostic overflow, event dropping |
...
| **DiagnosticEvent** | Structured event from the **Diagnostic Stream**: `{code: DiagnosticCode, severity: Severity, monotonicMillis, wallClockMillis, droppedCount, payload: Map<String, Any>}`. Four severity levels: `FATAL` (terminal), `ERROR` (app should act), `WARNING` (degraded), `INFO` (observability). Apps filter by severity without knowing every code. | Log event, debug event |
| **Diagnostic Timestamp** | Each `DiagnosticEvent` carries dual timestamps: `monotonicMillis` (millis since boot — `System.nanoTime()/1M` on Android, `ProcessInfo.systemUptime×1000` on iOS) for interval calculations and cross-event ordering; `wallClockMillis` (epoch millis — `System.currentTimeMillis()` on Android, `Date().timeIntervalSince1970×1000` on iOS) for human-readable log correlation. 16 bytes per event. | Event time, timestamp |
