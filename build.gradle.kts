...
// Force all security overrides at the project-dependency level too.
// AGP's UTP test infrastructure and other plugin runtime paths expose these
// libraries as project-level configurations — buildscript {} above is insufficient
// for those paths. allprojects {} registers live hooks that apply to every
// configuration any subproject or plugin creates, including AGP-internal ones.
allprojects {
    configurations.all {
        resolutionStrategy.eachDependency {
            if (requested.group == "io.netty") {
