...
class AndroidEd25519FallbackTest {
    ...
    fun `fallback matches RFC 8032 test vector 1`() {
        ...
        assertTrue(isValid, "The fallback should verify the RFC 8032 reference signature it produced")
    }
    ...
    fun `fallback signatures verify with JCA Ed25519`() {
        ...
        // Assert
        assertTrue(isValid, "Fallback signatures must remain interoperable with JCA Ed25519 verifiers")
    }
    ...
    fun `fallback verifies JCA Ed25519 signatures`() {
        ...
        // Assert
        assertTrue(isValid, "The fallback verifier must accept interoperable JCA Ed25519 signatures")
    }

    @Test
    fun `fallback provider is selected when JCA Ed25519 is missing`() {
        // Arrange
        val provider = AndroidCryptoProviderFactory.create(
            capabilityReport = AndroidJcaCapabilityReport(
                supportsX25519 = true,
        ...
        assertTrue(isValid, "Fallback-backed providers must expose fully working Ed25519 operations")
        assertTrue(
    ...
    fun `fallback rejects signatures with non canonical S component`() {
        ...
        val keyPair = fallback.deriveKeyPair(privateKey)
        val signature = fallback.sign(privateKey, message).also { tampered ->
        ...
        // Assert
        assertFalse(isValid, "The fallback must reject malleable signatures where S is not canonical")
    }
    ...
    private fun signWithJca(privateKey: ByteArray, message: ByteArray): ByteArray {
        ...
        signer.initSign(
            KeyFactory.getInstance("Ed25519").generatePrivate(
                PKCS8EncodedKeySpec(ED25519_PKCS8_PREAMBLE + privateKey),
            ),
        )
    ...
    private fun verifyWithJca(publicKey: ByteArray, message: ByteArray, signature: ByteArray): Boolean {
        val verifier = Signature.getInstance("Ed25519")
        verifier.initVerify(
            KeyFactory.getInstance("Ed25519").generatePublic(
                X509EncodedKeySpec(ED25519_X509_PREAMBLE + publicKey),
            ),
        )
