...
class TransferEngineTest {
    ...
    fun sendAndReceiveAllAcksCompletes() {
        ...
        val handle = engine.beginSend(msgHex, msgId, payload, chunkSize = 100)
        ...
        val update = engine.onAck(msgHex, ackSeq = 0, sackBitmask = 0uL)
    ...
    fun inboundChunksReassembleCorrectly() {
        ...
        val r0 = engine.onChunkReceived(msgHex, seqNum = 0, totalChunks = 2, chunk0)
        assertIs<ChunkAcceptResult.Ack>(r0)
        assertEquals(0, r0.ackSeq)

        val r1 = engine.onChunkReceived(msgHex, seqNum = 1, totalChunks = 2, chunk1)
    ...
    fun outOfOrderChunksProduceSackBitmask() {
        ...
        val r2 = engine.onChunkReceived(msgHex, seqNum = 2, totalChunks = 4, "c".encodeToByteArray())
        assertIs<ChunkAcceptResult.Ack>(r2)
        // ackSeq is -1 or 0 (nothing contiguous from 0), bitmask has bit for chunk 2

        // Receive chunk 0
        val r0 = engine.onChunkReceived(msgHex, seqNum = 0, totalChunks = 4, "a".encodeToByteArray())
        assertIs<ChunkAcceptResult.Ack>(r0)
        assertEquals(0, r0.ackSeq) // chunk 0 is contiguous

        // Receive chunk 1 + 3 → complete
        engine.onChunkReceived(msgHex, seqNum = 1, totalChunks = 4, "b".encodeToByteArray())
        val r3 = engine.onChunkReceived(msgHex, seqNum = 3, totalChunks = 4, "d".encodeToByteArray())
    ...
    fun emptyPayloadProducesSingleChunk() {
        ...
        val handle = engine.beginSend(msgHex, msgId, ByteArray(0), chunkSize = 100)
    ...
    fun ackForUnknownTransferReturnsUnknown() {
        val engine = createEngine()
        val result = engine.onAck("nonexistent", 0, 0uL)
    ...
    fun sweepStaleRemovesOldTransfers() {
        ...
        val msgId = ByteArray(16)
        val msgHex = "stale01"
        engine.beginSend(msgHex, msgId, "data".encodeToByteArray(), chunkSize = 100)
    ...
    fun sweepStaleRemovesOldReassemblies() {
        ...
        engine.onChunkReceived("reassembly01", seqNum = 0, totalChunks = 3, "a".encodeToByteArray())
    ...
    fun bufferBytesTracksUsage() {
        val engine = createEngine()
        val msgId = ByteArray(16)
        engine.beginSend("out01", msgId, "hello".encodeToByteArray(), chunkSize = 100)
        assertTrue(engine.outboundBufferBytes() > 0)

        engine.onChunkReceived("in01", seqNum = 0, totalChunks = 2, "world".encodeToByteArray())
    ...
    fun clearAllResetsState() {
        val engine = createEngine()
        val msgId = ByteArray(16)
        engine.beginSend("out01", msgId, "data".encodeToByteArray(), chunkSize = 100)
        engine.onChunkReceived("in01", seqNum = 0, totalChunks = 2, "data".encodeToByteArray())
