...
class MeshEngineLargeTransferTerminalSupportTest {
    @Test
    fun `complete returns sent and clears frames and removes the session`() =
        runBlocking<Unit> {
            // Arrange
    ...
    @Test
    fun `fail returns unreachable when no route was available`() =
        runBlocking<Unit> {
            // Arrange
            val diagnostics = mutableListOf<RecordedLargeTransferTerminalDiagnostic>()
            val clearedOutboundFrames = mutableListOf<Pair<PeerId, String>>()
    ...
    @Test
    fun `fail returns transfer timed out when a route was previously available`() =
        runBlocking<Unit> {
            // Arrange
            val diagnostics = mutableListOf<RecordedLargeTransferTerminalDiagnostic>()
            val clearedOutboundFrames = mutableListOf<Pair<PeerId, String>>()
    ...
    @Test
    fun `abort returns transfer aborted and clears frames and removes the session`() =
        runBlocking<Unit> {
            // Arrange
            val clearedOutboundFrames = mutableListOf<Pair<PeerId, String>>()
            val outboundTransfers = mutableMapOf<String, OutboundTransferSession>()
