...
class CrossPlatformParityTest {
    @Test
    fun `android and ios expose matching lifecycle transitions and power diagnostic payloads`() = runBlocking {
        // Arrange
        ...
        val androidDiagnosticsDeferred = async(start = CoroutineStart.UNDISPATCHED) {
            withTimeout(1_000) { androidApi.diagnosticEvents.take(5).toList() }
        }
        val iosDiagnosticsDeferred = async(start = CoroutineStart.UNDISPATCHED) {
            withTimeout(1_000) { iosApi.diagnosticEvents.take(5).toList() }
        }

        // Act
        val androidResults = listOf(
            androidApi.start(),
            androidApi.also { it.updateBattery(level = 0.20f, isCharging = false) }.state.value,
            androidApi.pause(),
            androidApi.resume(),
            androidApi.stop(),
        )
        val iosResults = listOf(
            iosApi.start(),
        ...
        assertEquals(androidDiagnostics.last().metadata.keys, iosDiagnostics.last().metadata.keys)
        assertEquals(androidDiagnostics.last().metadata["tier"], iosDiagnostics.last().metadata["tier"])
    }
    ...
    fun `shared diagnostic and failure catalogs stay aligned`() {
        ...
        val expectedFailureReasons = listOf(
            "PAYLOAD_TOO_LARGE",
            "TRANSFER_TIMED_OUT",
            "TRANSFER_ABORTED",
            "UNREACHABLE",
        ...
        val expectedExceptionCategories = listOf(
        ...
        val actualExceptionCategories = listOf(
