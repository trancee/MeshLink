...
class MeshLinkApiContractTest {
    private fun installIosFactoryTestBridge(): Unit {
        ...
        fun nextBytes(size: Int): ByteArray {
            return ByteArray(size) { index -> ((counter + index) and 0xFF).toByte() }.also { counter += 1 }
        }

        IosCryptoBridge.install(
            randomBytes = { size -> nextBytes(size) },
            sha256 = { input -> ByteArray(32) { index -> input.getOrElse(index) { index.toByte() } } },
            hmacSha256 = { _, data -> ByteArray(32) { index -> data.getOrElse(index) { index.toByte() } } },
            generateX25519KeyPair = {
                IosCryptoRawKeyPair(
                    privateKey = nextBytes(32),
                    publicKey = nextBytes(32),
                )
            },
            ...
            ed25519Sign = { _, message -> ByteArray(64) { index -> message.getOrElse(index % maxOf(1, message.size)) { 0 } } },
