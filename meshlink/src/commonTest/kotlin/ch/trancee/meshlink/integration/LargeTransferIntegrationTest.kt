...
class LargeTransferIntegrationTest {
    @Test
    fun `a 64 KiB payload can cross a relay hop when the network requires chunking`() = runBlocking {
        // Arrange
    ...
    @Test
    fun `a large transfer resumes after the active route changes before the retry deadline expires`() = runBlocking {
        // Arrange
        val harness = MeshTestHarness()
        val sender = harness.createNode("peer-a")
    ...
    @Test
    fun `duplicate and out-of-order chunk delivery does not corrupt or redeliver the payload`() = runBlocking {
        // Arrange
        val harness = MeshTestHarness()
        val sender = harness.createNode("peer-a")
        val relay = harness.createNode("peer-b")
