...
internal sealed class WireFrame {
    internal class Hello internal constructor(
        public val peerId: PeerId,
        public val helloIntervalMillis: Int,
    ) : WireFrame()

    internal class Ihu internal constructor(
        public val peerId: PeerId,
        public val receiveCost: Int,
    ) : WireFrame()

    internal class RouteUpdate internal constructor(
    ...
    internal class SeqNoRequest internal constructor(
        public val destinationPeerId: PeerId,
        public val requestedSeqNo: Long,
    ) : WireFrame()

    internal class RouteDigest internal constructor(
        public val peerId: PeerId,
        digest: ByteArray,
    ) : WireFrame() {
    ...
    internal class TransferChunk internal constructor(
        public val transferId: String,
    ...
    internal class TransferAck internal constructor(
        public val transferId: String,
    ...
    internal class TransferComplete internal constructor(
        public val transferId: String,
    ) : WireFrame()

    internal class TransferAbort internal constructor(
        public val transferId: String,
...
internal object WireCodec {
    ...
    internal fun decodePayload(type: WireEnvelopeType, payload: ByteArray): WireFrame {
        ...
        return when (type) {
            WireEnvelopeType.HELLO -> WireFrame.Hello(
                peerId = PeerId(requireString(table, HELLO_PEER_ID_FIELD_INDEX, "HELLO.peerId")),
                helloIntervalMillis = table.readInt(HELLO_INTERVAL_FIELD_INDEX),
            )

            WireEnvelopeType.IHU -> WireFrame.Ihu(
            ...
            WireEnvelopeType.ROUTE_UPDATE -> WireFrame.RouteUpdate(
                destinationPeerId = PeerId(requireString(table, ROUTE_UPDATE_DESTINATION_FIELD_INDEX, "ROUTE_UPDATE.destinationPeerId")),
                nextHopPeerId = PeerId(requireString(table, ROUTE_UPDATE_NEXT_HOP_FIELD_INDEX, "ROUTE_UPDATE.nextHopPeerId")),
            ...
            WireEnvelopeType.ROUTE_RETRACTION -> WireFrame.RouteRetraction(
                destinationPeerId = PeerId(requireString(table, ROUTE_RETRACTION_DESTINATION_FIELD_INDEX, "ROUTE_RETRACTION.destinationPeerId")),
                seqNo = table.readLong(ROUTE_RETRACTION_SEQ_NO_FIELD_INDEX),
            )

            WireEnvelopeType.SEQNO_REQUEST -> WireFrame.SeqNoRequest(
                destinationPeerId = PeerId(requireString(table, SEQNO_REQUEST_DESTINATION_FIELD_INDEX, "SEQNO_REQUEST.destinationPeerId")),
                requestedSeqNo = table.readLong(SEQNO_REQUEST_SEQ_NO_FIELD_INDEX),
            )

            WireEnvelopeType.ROUTE_DIGEST -> WireFrame.RouteDigest(
                peerId = PeerId(requireString(table, ROUTE_DIGEST_PEER_ID_FIELD_INDEX, "ROUTE_DIGEST.peerId")),
                digest = requireByteVector(table, ROUTE_DIGEST_DIGEST_FIELD_INDEX, "ROUTE_DIGEST.digest"),
            )

            WireEnvelopeType.MESSAGE -> WireFrame.Message(
                messageId = requireString(table, MESSAGE_ID_FIELD_INDEX, "MESSAGE.messageId"),
                originPeerId = PeerId(requireString(table, MESSAGE_ORIGIN_FIELD_INDEX, "MESSAGE.originPeerId")),
                destinationPeerId = PeerId(requireString(table, MESSAGE_DESTINATION_FIELD_INDEX, "MESSAGE.destinationPeerId")),
                priority = priorityFromCode(table.readByte(MESSAGE_PRIORITY_FIELD_INDEX)),
                ttlMillis = table.readInt(MESSAGE_TTL_FIELD_INDEX),
                encryptedPayload = requireByteVector(table, MESSAGE_PAYLOAD_FIELD_INDEX, "MESSAGE.encryptedPayload"),
            )
            ...
            WireEnvelopeType.TRANSFER_CHUNK -> WireFrame.TransferChunk(
                transferId = requireString(table, TRANSFER_CHUNK_ID_FIELD_INDEX, "TRANSFER_CHUNK.transferId"),
                chunkIndex = table.readInt(TRANSFER_CHUNK_INDEX_FIELD_INDEX),
                payload = requireByteVector(table, TRANSFER_CHUNK_PAYLOAD_FIELD_INDEX, "TRANSFER_CHUNK.payload"),
            )

            WireEnvelopeType.TRANSFER_ACK -> WireFrame.TransferAck(
                transferId = requireString(table, TRANSFER_ACK_ID_FIELD_INDEX, "TRANSFER_ACK.transferId"),
                highestContiguousAck = table.readInt(TRANSFER_ACK_HIGHEST_CONTIGUOUS_FIELD_INDEX),
                selectiveRanges = requireByteVector(table, TRANSFER_ACK_SELECTIVE_RANGES_FIELD_INDEX, "TRANSFER_ACK.selectiveRanges"),
            )

            WireEnvelopeType.TRANSFER_COMPLETE -> WireFrame.TransferComplete(
                transferId = requireString(table, TRANSFER_COMPLETE_ID_FIELD_INDEX, "TRANSFER_COMPLETE.transferId"),
            )

            WireEnvelopeType.TRANSFER_ABORT -> WireFrame.TransferAbort(
                transferId = requireString(table, TRANSFER_ABORT_ID_FIELD_INDEX, "TRANSFER_ABORT.transferId"),
    ...
    private fun encodePayload(frame: WireFrame): ByteArray {
        return when (frame) {
            is WireFrame.Hello -> FlatBufferTableBuilder(fieldCount = HELLO_FIELD_COUNT)
                .addString(HELLO_PEER_ID_FIELD_INDEX, frame.peerId.value)
                .addInt(HELLO_INTERVAL_FIELD_INDEX, frame.helloIntervalMillis)
                .finish()

            is WireFrame.Ihu -> FlatBufferTableBuilder(fieldCount = IHU_FIELD_COUNT)
                .addString(IHU_PEER_ID_FIELD_INDEX, frame.peerId.value)
                .addInt(IHU_RECEIVE_COST_FIELD_INDEX, frame.receiveCost)
                .finish()

            is WireFrame.RouteUpdate -> FlatBufferTableBuilder(fieldCount = ROUTE_UPDATE_FIELD_COUNT)
            ...
            is WireFrame.RouteRetraction -> FlatBufferTableBuilder(fieldCount = ROUTE_RETRACTION_FIELD_COUNT)
                .addString(ROUTE_RETRACTION_DESTINATION_FIELD_INDEX, frame.destinationPeerId.value)
                .addLong(ROUTE_RETRACTION_SEQ_NO_FIELD_INDEX, frame.seqNo)
                .finish()

            is WireFrame.SeqNoRequest -> FlatBufferTableBuilder(fieldCount = SEQNO_REQUEST_FIELD_COUNT)
            ...
            is WireFrame.RouteDigest -> FlatBufferTableBuilder(fieldCount = ROUTE_DIGEST_FIELD_COUNT)
                .addString(ROUTE_DIGEST_PEER_ID_FIELD_INDEX, frame.peerId.value)
                .addByteVector(ROUTE_DIGEST_DIGEST_FIELD_INDEX, frame.digest)
                .finish()

            is WireFrame.Message -> FlatBufferTableBuilder(fieldCount = MESSAGE_FIELD_COUNT)
            ...
            is WireFrame.TransferChunk -> FlatBufferTableBuilder(fieldCount = TRANSFER_CHUNK_FIELD_COUNT)
            ...
            is WireFrame.TransferAck -> FlatBufferTableBuilder(fieldCount = TRANSFER_ACK_FIELD_COUNT)
            ...
            is WireFrame.TransferComplete -> FlatBufferTableBuilder(fieldCount = TRANSFER_COMPLETE_FIELD_COUNT)
                .addString(TRANSFER_COMPLETE_ID_FIELD_INDEX, frame.transferId)
                .finish()

            is WireFrame.TransferAbort -> FlatBufferTableBuilder(fieldCount = TRANSFER_ABORT_FIELD_COUNT)
    ...
    private fun priorityFromCode(code: Byte): DeliveryPriority {
        return when (code.toInt()) {
            ...
            else -> throw IllegalStateException("Unknown delivery priority code ${code.toInt() and 0xFF}")
        }
    }

    private fun requireString(table: FlatBufferTable, fieldIndex: Int, fieldName: String): String {
        return table.readString(fieldIndex)
            ?: throw IllegalStateException("$fieldName is missing")
    }

    private fun requireByteVector(table: FlatBufferTable, fieldIndex: Int, fieldName: String): ByteArray {
