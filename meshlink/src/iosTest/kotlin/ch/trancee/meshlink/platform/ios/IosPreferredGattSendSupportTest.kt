package ch.trancee.meshlink.platform.ios

import ch.trancee.meshlink.api.PeerId
import ch.trancee.meshlink.engine.DirectWireFrame
import ch.trancee.meshlink.transport.BleDiscoveryPlatformFamily
import ch.trancee.meshlink.transport.OutboundFrame
import ch.trancee.meshlink.transport.TransportMode
import ch.trancee.meshlink.transport.TransportSendResult
import kotlin.test.Test
import kotlin.test.assertEquals
import kotlin.test.assertNull
import kotlinx.coroutines.runBlocking

class IosPreferredGattSendSupportTest {
    @Test
    fun sendViaPreferredGattNotifyLinkOrNullSkipsHandshakeFrames(): Unit = runBlocking {
        // Arrange
        val fixture = IosPreferredGattSendFixture()
        val frame =
            OutboundFrame(
                peerId = fixture.context.hintPeerId,
                payload = DirectWireFrame.HandshakeMessage1(byteArrayOf(0x01)).encode(),
            )

        // Act
        val result =
            fixture.run(frame = frame, link = FakeIosPreferredGattSendLink(enqueueResult = true))

        // Assert
        assertNull(result)
        assertEquals(0, fixture.currentLinkCalls)
    }

    @Test
    fun sendViaPreferredGattNotifyLinkOrNullSkipsSamePlatformDataFramesWithoutGattPreference():
        Unit = runBlocking {
        // Arrange
        val fixture =
            IosPreferredGattSendFixture(remotePlatformFamily = BleDiscoveryPlatformFamily.IOS)
        val frame =
            OutboundFrame(
                peerId = fixture.context.hintPeerId,
                payload = DirectWireFrame.Data(ByteArray(8)).encode(),
            )

        // Act
        val result =
            fixture.run(frame = frame, link = FakeIosPreferredGattSendLink(enqueueResult = true))

        // Assert
        assertNull(result)
        assertEquals(0, fixture.currentLinkCalls)
    }

    @Test
    fun sendViaPreferredGattNotifyLinkOrNullAllowsSamePlatformDataFramesWhenGattIsPreferred():
        Unit = runBlocking {
        // Arrange
        val fixture =
            IosPreferredGattSendFixture(remotePlatformFamily = BleDiscoveryPlatformFamily.IOS)
        val frame =
            OutboundFrame(
                peerId = fixture.context.hintPeerId,
                payload = DirectWireFrame.Data(ByteArray(8)).encode(),
                preferredMode = TransportMode.GATT,
            )

        // Act
        val result =
            fixture.run(frame = frame, link = FakeIosPreferredGattSendLink(enqueueResult = true))

        // Assert
        assertEquals(TransportSendResult.Delivered, result)
        assertEquals(1, fixture.currentLinkCalls)
    }

    @Test
    fun sendViaPreferredGattNotifyLinkOrNullFallsBackWhenTheSideLinkIsUnavailable(): Unit =
        runBlocking {
            // Arrange
            val fixture = IosPreferredGattSendFixture()
            val frame =
                OutboundFrame(
                    peerId = fixture.context.hintPeerId,
                    payload = DirectWireFrame.Data(ByteArray(8)).encode(),
                )

            // Act
            val result = fixture.run(frame = frame, link = null)

            // Assert
            assertNull(result)
            assertEquals(1, fixture.currentLinkCalls)
        }

    @Test
    fun sendViaPreferredGattNotifyLinkOrNullFallsBackWhenEnqueueReturnsFalse(): Unit = runBlocking {
        // Arrange
        val fixture = IosPreferredGattSendFixture()
        val frame =
            OutboundFrame(
                peerId = fixture.context.hintPeerId,
                payload = DirectWireFrame.Data(ByteArray(8)).encode(),
            )

        // Act
        val result =
            fixture.run(frame = frame, link = FakeIosPreferredGattSendLink(enqueueResult = false))

        // Assert
        assertNull(result)
        assertEquals(1, fixture.currentLinkCalls)
    }

    @Test
    fun sendViaPreferredGattNotifyLinkOrNullFallsBackWhenEnqueueThrows(): Unit = runBlocking {
        // Arrange
        val fixture = IosPreferredGattSendFixture()
        val frame =
            OutboundFrame(
                peerId = fixture.context.hintPeerId,
                payload = DirectWireFrame.Data(ByteArray(8)).encode(),
            )

        // Act
        val result =
            fixture.run(
                frame = frame,
                link = FakeIosPreferredGattSendLink(enqueueFailure = IllegalStateException("boom")),
            )

        // Assert
        assertNull(result)
        assertEquals(1, fixture.currentLinkCalls)
    }
}

private class IosPreferredGattSendFixture(
    remotePlatformFamily: BleDiscoveryPlatformFamily = BleDiscoveryPlatformFamily.ANDROID
) {
    val context =
        IosPreferredGattSendContext(
            hintPeerId = PeerId("peer-ios"),
            localPlatformFamily = BleDiscoveryPlatformFamily.IOS,
            remotePlatformFamily = remotePlatformFamily,
        )
    var currentLinkCalls: Int = 0

    suspend fun run(
        frame: OutboundFrame,
        link: FakeIosPreferredGattSendLink?,
    ): TransportSendResult? {
        return sendViaPreferredGattNotifyLinkOrNull(
            frame = frame,
            context = context,
            dependencies =
                IosPreferredGattSendDependencies(
                    currentLink = {
                        currentLinkCalls += 1
                        link
                    },
                    log = {},
                ),
        )
    }
}

private class FakeIosPreferredGattSendLink(
    private val enqueueResult: Boolean = true,
    private val enqueueFailure: Throwable? = null,
) : IosPreferredGattSendLink {
    override suspend fun enqueue(payload: ByteArray): Boolean {
        enqueueFailure?.let { throw it }
        return enqueueResult
    }
}
