...
class L2capOutputWriterTest {
    ...
    fun writeFailsWhenTheStreamIsAlreadyClosed(): Unit = runBlocking {
        // Arrange
        val adapter = FakeL2capOutputStreamAdapter(currentStreamStatus = NSStreamStatusClosed)
        val timing = advancingWriteTiming(startAtMs = 100L, stepMs = 10L, activePollIntervalMs = 0L)
        val writer = L2capOutputWriter(outputStream = adapter, timing = timing)
    ...
    fun writeFailsWhenReadyFalsePersistsPastTheStallTimeout(): Unit = runBlocking {
        // Arrange
        val adapter = FakeL2capOutputStreamAdapter(hasSpaceResults = List(4) { false })
        val timing =
...
private class FakeL2capOutputStreamAdapter(
