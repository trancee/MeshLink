...
class NoiseXXHandshakeManagerTest {
    ...
    fun `noise xx handshake derives matching transport keys`() {
        ...
        assertEquals(
            responderIdentity.x25519KeyPair.publicKey.toList(),
            initiatorResult.remoteStaticPublicKey.toList(),
        )
        assertEquals(
            initiatorIdentity.x25519KeyPair.publicKey.toList(),
            responderResult.remoteStaticPublicKey.toList(),
        )
    }
