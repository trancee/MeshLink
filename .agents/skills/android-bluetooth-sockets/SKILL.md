...
description: Complete reference for Android Bluetooth socket-based communication from the official API docs. Covers both BR/EDR (Classic) RFCOMM sockets and BLE L2CAP Connection-oriented Channels (CoC). Server side — BluetoothServerSocket created via BluetoothAdapter listen methods (listenUsingRfcommWithServiceRecord for secure RFCOMM with SDP registration, listenUsingInsecureRfcommWithServiceRecord for insecure RFCOMM, listenUsingL2capChannel for secure LE CoC with dynamic PSM, listenUsingInsecureL2capChannel for insecure LE CoC, listenUsingSocketSettings for unified API 36+ config). Client side — BluetoothSocket created via BluetoothDevice create methods (createRfcommSocketToServiceRecord for secure RFCOMM via SDP UUID lookup, createInsecureRfcommSocketToServiceRecord for insecure RFCOMM, createL2capChannel for secure LE CoC by PSM, createInsecureL2capChannel for insecure LE CoC, createUsingSocketSettings for unified API 36+ config). Socket lifecycle (accept blocks until connection, connect blocks until established, getInputStream/getOutputStream for IO, close for cleanup, thread-safe close aborts blocking calls). BluetoothSocketSettings (API 36) — Builder pattern for unified socket creation with setSocketType (TYPE_RFCOMM or TYPE_LE), setRfcommUuid, setRfcommServiceName, setL2capPsm (128-255 for LE), setAuthenticationRequired, setEncryptionRequired. Socket types (TYPE_RFCOMM streaming serial, TYPE_L2CAP classic L2CAP, TYPE_LE L2CAP on LE transport with credit-based flow control, TYPE_SCO audio). PSM management (dynamic assignment via getPsm, app-defined PSM exchange mechanism, released on close/BT off/process death). SDP (automatic registration with UUID+name+channel, auto-removed on close). Permissions (BLUETOOTH_CONNECT for API 31+, BLUETOOTH for older). Threading (accept/connect are blocking — run on background threads, close from another thread to abort). Critical rules (cancelDiscovery before connect, close ServerSocket after accepting when single-connection, close BluetoothSocket when done, ServerSocket close does NOT close accepted sockets). Use when implementing Bluetooth socket communication, setting up RFCOMM server/client, using L2CAP CoC channels, choosing between secure and insecure sockets, managing socket lifecycle, or any Android BluetoothServerSocket/BluetoothSocket question.
---
