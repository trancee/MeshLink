---
name: hmac-rfc2104
description: Complete reference for RFC 2104 — HMAC Keyed-Hashing for Message Authentication. Covers the HMAC construction using any iterative hash function H with a secret key K. Algorithm is H((K' XOR opad) || H((K' XOR ipad) || text)) where ipad=0x36×B and opad=0x5C×B, K' is K zero-padded to B bytes (or H(K) if K > B). Includes key handling (min L bytes recommended, keys > B hashed first, must be random, periodic refresh), precomputation optimization (cache intermediate hash state after processing padded key blocks — saves two compression calls per message), truncation rules (min half of hash output and min 80 bits, notation HMAC-H-t), security analysis (relies on collision resistance with secret IV and compression function MAC property — weaker than general collision resistance so hash collision attacks like MD5 do NOT break HMAC-MD5; birthday attack is strongest known requiring 2^(L/2) online messages with same key — totally impractical; transient effect means breaking MAC doesn't compromise past authentications), and HMAC-MD5 test vectors. Use when implementing HMAC, understanding the inner/outer hash construction, key length requirements, precomputation optimization, truncated MAC output, HMAC security properties, or any question about RFC 2104.
---
