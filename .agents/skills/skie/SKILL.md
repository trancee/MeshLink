---
name: skie
description: Complete reference for SKIE (by Touchlab) — a Kotlin Native compiler plugin that improves Swift interop for Kotlin Multiplatform (KMP) projects. SKIE modifies the Xcode Framework produced by the Kotlin compiler to restore language features lost in the Kotlin→Objective-C→Swift bridge. Covers all features (enums with exhaustive switching, sealed classes with onEnum(of:), default arguments via generated overloads, global functions without FileKt namespace, interface extensions as member-style calls, overloaded function disambiguation, suspend functions as proper Swift async with two-way cancellation, Flows as AsyncSequence with preserved generics, Swift code bundling into Kotlin frameworks), preview features (Flows in SwiftUI with Observing view and collect modifier, Combine bridge with Future and Publisher), installation (Gradle plugin co.touchlab.skie version 0.10.11, apply only in framework-producing module, runs on all exported dependencies), configuration (Gradle skie{} DSL with group-based prefix matching and overridesAnnotations, annotation-based @FlowInterop.Enabled/@Disabled per-declaration, configuration-annotations dependency co.touchlab.skie:configuration-annotations, isEnabled.set(false) to disable entirely), migration (existing projects need Swift-side fixes for enum case name changes, exhaustive switch default removal, sealed class onEnum(of:) adoption, Flow type changes, suspend function __ prefix for overrides), compatibility (Kotlin 2.0.0–2.3.10, Swift 5.8+ / Xcode 14.3+, new Kotlin support within days of release), and limitations (enum generics use original __Enum type, sealed interface Hashable only when all children are classes, default arguments disabled by default due to O(2^n) overload generation capped at 5 params, suspend function generic class members require skie() wrapper, Flow type casting unsafe at runtime, custom Flow types unsupported). Use when installing or configuring SKIE, writing Swift code that consumes Kotlin enums/sealed classes/suspend functions/Flows, migrating an existing project to SKIE, debugging SKIE-related Swift compilation errors, choosing which SKIE features to enable/disable, bundling custom Swift code into a Kotlin framework, using Kotlin Flows in SwiftUI or Combine, or any question about SKIE and Kotlin-Swift interop.
---
