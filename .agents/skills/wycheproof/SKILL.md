---
name: wycheproof
description: Complete reference for Project Wycheproof (C2SP/wycheproof) — a community-managed repository of 340+ JSON test vector files covering 30+ cryptographic algorithms. Detects 80+ attack categories including invalid curve attacks, biased nonces, Bleichenbacher's attacks, padding oracles, signature malleability, and more. Used by OpenSSL, BoringSSL, Go crypto, pyca/cryptography, RustCrypto, swift-crypto, NSS, LibreSSL, Botan, Zig, and many others. Covers AEAD (AES-GCM, AES-GCM-SIV, AES-EAX, AES-CCM, AES-SIV-CMAC, ChaCha20-Poly1305, XChaCha20-Poly1305, AEGIS-128/128L/256, AES-CBC-PKCS5, AES-XTS, ASCON), digital signatures (ECDSA with 73 curve/hash files including secp256k1 Bitcoin variant, EdDSA Ed25519/Ed448, DSA, RSA PKCS#1v1.5, RSA-PSS, ML-DSA/Dilithium), key exchange (ECDH on multiple curves with ASN/PEM/ecpoint/webcrypto encodings, X25519, X448, ML-KEM/Kyber), MACs (HMAC-SHA*, AES-CMAC, KMAC128/256, SipHash), KDFs (HKDF, PBKDF2), RSA encryption (OAEP, PKCS#1v1.5), key wrap (AES-KW, AES-KWP), format-preserving encryption (AES-FF1), BLS-12-381, primality testing, JSON Web Crypto/Encryption/Signature, plus ARIA, Camellia, SEED, SM4. Test vector JSON format (algorithm, schema, generatorVersion 0.9, numberOfTests, notes dictionary with bugType/description/effect/links/cves, testGroups array with per-group parameters and tests array). Data types (HexBytes hex-encoded, BigInt twos-complement hex big-endian, Asn hex-encoded possibly invalid ASN.1, Der valid DER hex, Pem PEM-encoded key). Result semantics (valid = MUST accept, invalid = MUST reject, acceptable = MAY accept or reject — check flags). Bug types (BASIC, AUTH_BYPASS, CONFIDENTIALITY, EDGE_CASE, SIGNATURE_MALLEABILITY, MALLEABILITY, BER_ENCODING, CAN_OF_WORMS, MISSING_STEP, KNOWN_BUG, WRONG_PRIMITIVE, MODIFIED_PARAMETER, LEGACY, FUNCTIONALITY, WEAK_PARAMS, DEFINED). Format conventions (hash function names SHA-1/SHA-256/SHA-384/SHA-512/SHA3-*/SHAKE*, elliptic curve names with OIDs — secp256r1/P-256/secp384r1/P-384/secp521r1/P-521/secp256k1/brainpoolP*/curve25519/curve448/edwards25519/edwards448). All test group types (AeadTestGroup, DaeadTestGroup, EcdhTestGroup variants for ASN/PEM/ecpoint/webcrypto, EcdsaTestGroup/P1363/Bitcoin, EddsaTestGroup, HkdfTestGroup, MacTestGroup, XdhTestGroup/AsnComp/JwkComp, MlkemTestGroup, MldsaTestGroup, RsaesOaep/Pkcs1, KeywrapTestGroup, PrimalityTestGroup, and more). Use when writing tests against Wycheproof vectors, integrating crypto test vectors into CI, understanding what a specific test flag or bugType means, choosing which vector files cover a given algorithm, parsing the JSON structure for any test type, or validating a cryptographic implementation against known attacks.
---
