---
name: flatbuffers
description: Complete reference for Google FlatBuffers — an efficient cross-platform serialization library providing zero-copy access to serialized data without parsing or unpacking. Covers the schema language (.fbs IDL) including tables (fields with default/optional/required semantics), structs (inline fixed-layout, no versioning), scalars (8/16/32/64-bit signed/unsigned/float), vectors, strings (UTF-8, zero-terminated), enums (integer-backed), unions (tagged sum types with implicit _type field), namespaces, includes, root_type, file_identifier (4-byte magic), and file_extension. Schema evolution rules (add fields at end only or use explicit id attributes, deprecate but never remove fields, never change defaults, union evolution with discriminants). All attributes (id, deprecated, required, force_align, bit_flags, nested_flatbuffer, flexbuffer, key, hash, original_order). Binary format internals (little-endian, uoffset_t uint32 forward offsets, soffset_t to vtables, vtable sharing with voffset_t uint16 entries for size + field offsets, structs inline with alignment, vectors as count-prefixed contiguous elements, strings as null-terminated vectors of bytes, backward construction). The flatc compiler (language code generators for 15 languages including --kotlin, --binary/--json conversion, --conform for evolution checking, --gen-object-api, --gen-mutable, --grpc, --require-explicit-ids). FlexBuffers schema-less variant (self-describing, variable bit-width 8/16/32/64, typed/untyped vectors, maps with sorted keys and binary search, IndirectFloat/Double for sharing, root at end of buffer, blob/string/key types). Kotlin/JVM usage (flatbuffers-java runtime, property-style field access, companion object statics, JVM-only — no Kotlin Native/JS). Style guide (UpperCamelCase types, snake_case fields, UpperCamelCase enum values). Efficiency guidelines (prefer enums over strings, prefer structs over tables for fixed data, use smallest integer types, share repeated data). Use when writing .fbs schemas, generating code with flatc, understanding the binary wire format, evolving schemas safely, choosing between tables and structs, working with unions or FlexBuffers, debugging FlatBuffer encoding, or any FlatBuffers question.
---
