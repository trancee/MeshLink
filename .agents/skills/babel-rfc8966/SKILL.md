---
name: babel-rfc8966
description: Complete reference for RFC 8966 — The Babel Routing Protocol (IETF Standards Track, January 2021). Loop-avoiding distance-vector protocol for wired and wireless mesh networks. Covers the conceptual model (Bellman-Ford with feasibility conditions, sequenced routes, starvation recovery via seqno requests, multiple originators, overlapping prefixes), all data structures (interface/neighbour/source/route tables, pending seqno requests), protocol operation (neighbour acquisition via Hello/IHU, cost computation, feasibility condition with FD, route acquisition, metric computation, hold time, route selection with hysteresis, periodic and triggered updates, split horizon, route and seqno requests), wire format (UDP port 6696, packet structure, address encodings AE 0-3, all 11 TLV types with field layouts, sub-TLVs with mandatory bit, stateful parser), cost algorithms (k-out-of-j for wired, ETX for wireless, hysteresis), recommended default parameters, security (Babel-MAC RFC 8967, Babel-DTLS RFC 8968), extension mechanisms, route filtering, and stub implementations. Use when implementing Babel routing, debugging route convergence, understanding feasibility conditions, working with Babel TLV encoding, tuning Hello/Update intervals, or any question about RFC 8966.
---
