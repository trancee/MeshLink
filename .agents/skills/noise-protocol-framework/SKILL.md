...
description: Complete reference for the Noise Protocol Framework (Revision 34, July 2018) — a framework for crypto protocols based on Diffie-Hellman key agreement. Covers the handshake state machine (CipherState with k/n, SymmetricState with ck/h, HandshakeState with s/e/rs/re), message tokens (e, s, ee, es, se, ss, psk), processing rules (WriteMessage/ReadMessage sequentially process tokens, Split returns two CipherStates for transport), crypto function signatures (DH with DHLEN, ENCRYPT/DECRYPT AEAD with 32-byte key and 8-byte nonce and 16-byte auth tag, HASH with HASHLEN/BLOCKLEN, HMAC-HASH, HKDF 2 or 3 outputs), all handshake patterns (3 one-way N/K/X, 12 fundamental interactive NN/NK/NX/KN/KK/KX/XN/XK/XX/IN/IK/IX, 23 deferred variants with numeral-1 naming), pattern naming convention (first char = initiator static key status N/K/X/I, second char = responder N/K/X), payload security properties (source 0-2 authentication levels, destination 0-5 confidentiality levels including forward secrecy), identity hiding properties (9 levels from cleartext to encrypted-with-forward-secrecy-to-authenticated-party), protocol names (Noise_XX_25519_AESGCM_SHA256 format), PSK mode (psk0/psk1/psk2 modifiers, MixKeyAndHash, validity rule requiring ephemeral before encrypted data), compound protocols (fallback modifier, Noise Pipes with XX/IK/XXfallback for zero-RTT), advanced features (dummy keys, channel binding via GetHandshakeHash, Rekey for forward secrecy, out-of-order transport via SetNonce, half-duplex mode), concrete algorithms (25519/448 DH, ChaChaPoly/AESGCM cipher, SHA256/SHA512/BLAKE2s/BLAKE2b hash), security considerations (ephemeral key reuse catastrophic, nonce exhaustion at 2^64-1, static key reuse only with single hash, AESGCM 2^56 byte volume limit, misusing public keys as secrets, rollback attacks, hash collision transcript attacks), and application responsibilities (negotiation data, length fields, session termination, padding). Use when implementing a Noise protocol handshake, choosing a handshake pattern, understanding payload security properties, debugging handshake state, working with Noise Pipes or PSK mode, or any question about the Noise Protocol Framework.
---
