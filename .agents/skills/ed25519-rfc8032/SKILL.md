...
description: Complete reference for RFC 8032 — Edwards-Curve Digital Signature Algorithm (EdDSA). Covers all five instantiations (Ed25519, Ed25519ctx, Ed25519ph, Ed448, Ed448ph), the generic EdDSA algorithm with 11 parameters, curve equations (twisted and untwisted Edwards), key generation with scalar pruning (cofactor clearing, high-bit clamping), deterministic signing (nonce derived from prefix+message via SHA-512 or SHAKE256), verification with cofactor multiplication, point encoding/decoding (compressed y-coordinate with x sign bit, little-endian, square root recovery), point arithmetic in extended homogeneous coordinates (Ed25519) and projective coordinates (Ed448) with complete addition formulas, domain separation (dom2/dom4), PureEdDSA vs HashEdDSA trade-offs (collision resilience vs single-pass), security considerations (constant-time implementation, deterministic nonces, signature malleability S<L check, context usage, cofactor multiplication semantics, large message buffering), and test vectors for all five variants. Use when implementing Ed25519 or Ed448 signing/verification, working with EdDSA key generation, debugging point encoding/decoding, understanding scalar clamping, checking signature malleability, or any question about RFC 8032.
---
