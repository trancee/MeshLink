...
enum MeshLinkProofCryptoBridge {
    static func install() {
        CryptoBridge.shared.install(
            ...
            x25519: { privateKeyBytes, publicKeyBytes in
                do {
                    ...
                    fatalError("MeshLinkProofCryptoBridge X25519 failed: \(error)")
                }
            },
            ed25519Sign: { privateKeyBytes, message in
                do {
                    ...
                    fatalError("MeshLinkProofCryptoBridge Ed25519 sign failed: \(error)")
                }
            },
            ...
            chacha20Poly1305Seal: { keyBytes, nonceBytes, aadBytes, plaintext in
                do {
                    ...
                    fatalError("MeshLinkProofCryptoBridge ChaCha20-Poly1305 seal failed: \(error)")
                }
            },
            chacha20Poly1305Open: { keyBytes, nonceBytes, aadBytes, ciphertextAndTag in
                do {
                    ...
                    guard ciphertextData.count >= tagSize else {
                        fatalError("MeshLinkProofCryptoBridge ChaCha20-Poly1305 ciphertext is shorter than the authentication tag")
                    }
                    let key = SymmetricKey(data: keyBytes.toData())
                    let nonce = try ChaChaPoly.Nonce(data: nonceBytes.toData())
                    ...
                    fatalError("MeshLinkProofCryptoBridge ChaCha20-Poly1305 open failed: \(error)")
                }
            }
        )
    }
