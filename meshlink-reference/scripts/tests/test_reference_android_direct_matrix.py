...
class AndroidDirectMatrixScriptTests(unittest.TestCase):
    ...
    def test_main_writes_fleet_inventory_and_pair_report_with_fail_fast(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertIn("autonumber", report_text)
            self.assertIn("rect rgba(245, 247, 250, 0.55)", report_text)
            self.assertIn("rect rgba(236, 253, 245, 0.55)", report_text)
            self.assertIn("Transport: meshlink", report_text)
            self.assertIn("launch initial run", report_text)
            self.assertIn("wait for passive peer id", report_text)
            self.assertIn("discovery and route establishment", report_text)
            self.assertIn("send guided payload", report_text)
            self.assertIn("record failure and continue until the cap is reached", report_text)
