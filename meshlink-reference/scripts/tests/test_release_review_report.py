...
class ReleaseReviewReportTests(unittest.TestCase):
    ...
    def test_render_report_surfaces_verdict_counts_gate_math_summaries_and_retained_links(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertIn("Gate policy", html_text)
            self.assertIn("Retained policy thresholds come directly from report-data.json", html_text)
            self.assertIn("Failure count max", html_text)
