    ...
    LazyColumn(
        ...
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                Button(onClick = { store.exportCurrentSession(ExportPayloadPolicy.REDACTED_PREVIEW) }) { Text("Export redacted") }
                Button(onClick = { store.exportCurrentSession(ExportPayloadPolicy.FULL_PAYLOAD_OPT_IN) }) { Text("Export full payload") }
            }
        }
        item {
            if (uiState.lastExportPath != null) {
                Text(text = "Last export: ${uiState.lastExportPath}", style = MaterialTheme.typography.bodyMedium)
            }
        }
            ...
            Card(modifier = Modifier.fillMaxWidth()) {
                Column(modifier = Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    if (entry.payloadPreview != null) {
                        Text(text = "Preview: ${entry.payloadPreview}", style = MaterialTheme.typography.bodyMedium)
                    }
                }
            }
        }
