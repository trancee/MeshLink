...
public fun AdvancedControlsScreen(
    ...
    LazyColumn(
        ...
        item {
            Card(
                ...
                Column(modifier = Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Text(text = "Mesh state: ${uiState.meshStateLabel}", modifier = Modifier.testTag("advanced-mesh-state"))
                }
            }
        }
        item {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                androidx.compose.foundation.layout.Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    Button(onClick = viewModel::startMesh, enabled = lifecycleActions.startEnabled) { Text("Start") }
                    Button(onClick = viewModel::pauseMesh, enabled = lifecycleActions.pauseEnabled) { Text("Pause") }
                    Button(onClick = viewModel::resumeMesh, enabled = lifecycleActions.resumeEnabled) { Text("Resume") }
                    Button(onClick = viewModel::stopMesh, enabled = lifecycleActions.stopEnabled) { Text("Stop") }
                }
            }
        }
        item {
            Text(text = "Peers", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.SemiBold)
        }
        ...
        item {
            Button(onClick = viewModel::forgetSelectedPeer, enabled = uiState.canForgetPeer, modifier = Modifier.testTag("advanced-forget-peer")) {
                Text("Forget selected peer")
            }
        }
        item {
            Text(text = "Recent highlights", style = MaterialTheme.typography.titleLarge)
        }
