...
internal class ReferenceSurfaceSelectionRequest(
    val currentKind: ReferenceSessionKind,
    val activeRoute: ReferenceSurfaceId,
    val targetSurface: ReferenceSurfaceId,
)

internal sealed interface ReferenceSurfaceSelectionDecision {
    data class SelectSurface(val surface: ReferenceSurfaceId) : ReferenceSurfaceSelectionDecision

    data class RequireBoundary(val request: SessionBoundaryRequest) :
        ReferenceSurfaceSelectionDecision

    data class StartNewSession(val surface: ReferenceSurfaceId) : ReferenceSurfaceSelectionDecision
}

internal fun resolveSurfaceSelection(
    request: ReferenceSurfaceSelectionRequest
): ReferenceSurfaceSelectionDecision {
    return when {
        request.currentKind == ReferenceSessionKind.SUPPORTED_LIVE &&
            request.targetSurface == ReferenceSurfaceId.SOLO_EXPLORATION ->
            ReferenceSurfaceSelectionDecision.RequireBoundary(
                SessionBoundaryRequest.SupportedTo(request.targetSurface)
            )
        ...
        request.currentKind == ReferenceSessionKind.SOLO ||
            request.currentKind == ReferenceSessionKind.LAB ->
            when (request.targetSurface) {
                ...
                ReferenceSurfaceId.LAB -> {
                    if (request.targetSurface != request.activeRoute) {
                        ReferenceSurfaceSelectionDecision.RequireBoundary(
                            SessionBoundaryRequest.AlternativeTo(request.targetSurface)
                        )
                    } else {
                        ReferenceSurfaceSelectionDecision.SelectSurface(request.targetSurface)
                    }
                }

                else -> ReferenceSurfaceSelectionDecision.SelectSurface(request.targetSurface)
            }

        request.currentKind == ReferenceSessionKind.SUPPORTED_ENDED &&
            request.targetSurface == ReferenceSurfaceId.SOLO_EXPLORATION ->
            ReferenceSurfaceSelectionDecision.StartNewSession(request.targetSurface)
